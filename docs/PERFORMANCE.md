...
> **测试日期**：2026-09-09
> **插件版本**：`feat/pr-pppoeclient` @ `8ca924fcb`（VPP 基线：官方 master 快照 `v26.10-rc0~463-g743392f24`）
> **测试性质**：短时摸底（5 秒量级），用于判断当前实现的性能量级；
> 完整稳定性与压力测试计划使用 TRex 进行。

> 说明：测试机 `show version` 输出为 `v26.10-rc0~425-g3ee242713`，那是同步源码时残留的旧 git
> 元数据；已逐文件核对，测试所用插件源码与 `8ca924fcb` 完全一致。
